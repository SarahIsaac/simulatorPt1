...
public:
	...
	EventQueue(int cpu_c, double mix, double freq, int io, int cost)
	{
		srand(time(NULL));
		...
		while (!event_queue.empty())
		{
			...
			else
			{
				if (current_event.task.get_job().type == "cpu")
				{
					...
					while (!ready_set.task_queue.empty() && ready_set.idle_cpu_count > 0)
					{
	...
	void scheduleTask(Task task)
	{
		if (task.get_job().type == "cpu")
		{
			...
			{
				ready_set.add();
				int exec_time = task.get_job().duration + current_time + context_switch;
				CPUEvent e(exec_time, task);
				event_queue.push(e);
			}
		}
